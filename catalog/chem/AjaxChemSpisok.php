...
$send[0]->count=$VK->QRow("select count(id) from chem_catalog ".$find);
if($_GET['device']>0 or $_GET['vendor']>0 or $_GET['model']>0) $fCount="Найдено "; else $fCount="В каталоге ";
$fCount="<H6><A href='javascript:' onclick=chemAdd();>Внести новую схему в каталог</A></H6>".$fCount;
$send[0]->result=iconv("WINDOWS-1251","UTF-8",$fCount.$send[0]->count." схем".zEnd($send[0]->count));
$send[0]->page=0;

$spisok=$VK->QueryObjectArray("select * from chem_catalog ".$find." order by id ".($_GET['desc']==0?'desc':'')." limit ".(($_GET['page']-1)*20).",20");
if(count($spisok)>0)
	{
	...
	if(count($spisok)==20)
		{
		$count=$VK->QNumRows("select id from chem_catalog ".$find." limit ".($_GET['page']*20).",20");
		$_GET['page']++;
