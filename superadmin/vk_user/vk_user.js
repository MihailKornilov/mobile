G.spisok.unit = function (sp) {
  ...
  return "<TABLE cellpadding=0 cellspacing=0 width=100%><TR>" +
    ...
    "<TD class=about>" +
      ...
      (sp.admin == 1 ? "<SPAN class=admin>(админ)</SPAN>" : '') +
//      "<DIV class=img_del val=del_" + sp.num + "></DIV>" +
      "<DIV class=enter>" + sp.enter_last + "</DIV>" +

      (sp.ws_id > 0 ? "<DIV>ws: " + sp.ws_id + "</DIV>" : '') +
      "<DIV>Рег.: " + sp.dtime_add + "</DIV>" +
      "<DIV class=counts><A val=counts_" + sp.viewer_id + ">Активность..</A></DIV>" +
...
G.spisok.create({
  json:G.sa.vk_user,
  view:$("#spisok"),
  result:"Показан$show $count клиент$client",
  ends:{'$show':['', 'o'], '$client':['', 'а', 'ов']},
  result_view:$("#result"),
  limit:50
});
...
function del(n) {
  ...
  $.post("/superadmin/vk_user/AjaxVkUserDel.php?" + G.values, {viewer_id:sp.viewer_id, dtime:sp.dtime}, function () {
...
function counts(id, e) {
  ...
  $.post("/superadmin/vk_user/AjaxVkUserCounts.php?" + G.values, {viewer_id:id}, function (res) {
    var html = '';
