...
submit:function(){var c={op:"worker_del",viewer_id:b.attr("val")};a.process();$.post(AJAX_SETUP,c,function(b){b.success?(a.close(),_msg("Сотрудник удален."),$("#spisok").html(b.html)):a.abort()},"json")}})}).on("click","#setup-service-cartridge .add",function(){function b(){var b={op:"cartridge_add",name:$("#name").val(),cost_filling:_num($("#cost_filling").val()),cost_restore:_num($("#cost_restore").val()),cost_chip:_num($("#cost_chip").val())};b.name?(a.process(),$.post(AJAX_SETUP,b,function(b){b.success?
($("#spisok").html(b.html),a.close(),_msg("Внесено!")):a.abort()},"json")):(a.err("Не указано наименование"),$("#name").focus())}$(this);var a=_dialog({top:40,width:400,head:"Добавление нового картриджа",content:'<table class="setup-tab"><tr><td class="label"><b>Модель картриджа:</b><td><input type="text" id="name" /><tr><td class="label">Заправка:<td><input type="text" id="cost_filling" class="money" maxlength="11" /> руб.<tr><td class="label">Восстановление:<td><input type="text" id="cost_restore" class="money" maxlength="11" /> руб.<tr><td class="label">Замена чипа:<td><input type="text" id="cost_chip" class="money" maxlength="11" /> руб.</table>',
submit:b});$("#name").focus();$("#name,#cost_filling,#cost_restore,#cost_chip").keyEnter(b)}).on("click","#setup-service-cartridge .img_edit",function(){function b(){var a={op:"cartridge_edit",id:c,name:$("#name").val(),cost_filling:_num($("#cost_filling").val()),cost_restore:_num($("#cost_restore").val()),cost_chip:_num($("#cost_chip").val())};a.name?(f.process(),$.post(AJAX_SETUP,a,function(a){a.success?($("#spisok").html(a.html),f.close(),_msg("Изменено!")):f.abort()},"json")):(f.err("Не указано наименование"),
$("#name").focus())}for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var c=a.attr("val"),e=a.find(".name").html(),d=a.find(".filling").html(),g=a.find(".restore").html(),a=a.find(".chip").html(),f=_dialog({top:40,width:400,head:"Редактирование данных картриджа",content:'<table class="setup-tab"><tr><td class="label"><b>Модель картриджа:</b><td><input type="text" id="name" value="'+e+'" /><tr><td class="label">Заправка:<td><input type="text" id="cost_filling" class="money" maxlength="11" value="'+
d+'" /> руб.<tr><td class="label">Восстановление:<td><input type="text" id="cost_restore" class="money" maxlength="11" value="'+g+'" /> руб.<tr><td class="label">Замена чипа:<td><input type="text" id="cost_chip" class="money" maxlength="11" value="'+a+'" /> руб.</table>',butSubmit:"Сохранить",submit:b});$("#name").focus();$("#name,#cost_filling,#cost_restore,#cost_chip").keyEnter(b)}).on("click","#setup-service-cartridge .img_del",function(){var b=$(this),a=_dialog({top:90,width:300,head:"Удаление картриджа",
content:"<center><b>Подтвердите удаление картриджа.</b></center>",butSubmit:"Удалить",submit:function(){for(;"TR"!=b[0].tagName;)b=b.parent();var c={op:"cartridge_del",id:b.attr("val")};a.process();$.post(AJAX_SETUP,c,function(b){b.success?($("#spisok").html(b.html),a.close(),_msg("Удалено!")):a.abort()},"json")}})}).on("click","#setup_invoice .add",function(){function b(){var b={op:"invoice_add",name:$("#name").val(),about:$("#about").val()};b.name?(c.process(),$.post(AJAX_SETUP,b,function(b){b.success?
($(".spisok").html(b.html),c.close(),_msg("Внесено!")):(c.abort(),a(b.text))},"json")):(a("Не указано наименование"),$("#name").focus())}function a(a){c.bottom.vkHint({msg:"<SPAN class=red>"+a+"</SPAN>",top:-47,left:100,indent:50,show:1,remove:1})}$(this);var c=_dialog({width:400,head:"Добавление нового счёта",content:'<table class="setup-tab"><tr><td class="label">Наименование:<td><input id="name" type="text" maxlength="50" /><tr><td class="label topi">Описание:<td><textarea id="about"></textarea></table>',
...
a.abort()},"json")}})}).ready(function(){$("#setup_info").length&&($("#info_save").click(function(){var b=$(this),a={op:"info_save",org_name:$.trim($("#org_name").val())};a.org_name?(b.addClass("busy"),$.post(AJAX_SETUP,a,function(a){b.removeClass("busy");a.success&&_msg("Сохранено.")},"json")):(b.vkHint({msg:'<span class="red">Не указано название организации</span>',remove:1,indent:40,show:1,top:-57,left:-5}),$("#org_name").focus())}),$("#info_del").click(function(){var b=_dialog({top:150,width:300,
head:"Удаление мастерской",content:"<center>Вы действительно хотите<BR>удалить мастерскую и все данные?</center>",butSubmit:"&nbsp;&nbsp;&nbsp;&nbsp;Да&nbsp;&nbsp;&nbsp;&nbsp;",submit:function(){b.process();$.post(AJAX_SETUP,{op:"info_del"},function(a){a.success?location.href=URL:b.abort()},"json")}})}));$("#setup-service").length&&$(".s-cartridge-toggle").click(function(){for(var b=$(this),a=b,c={op:"cartridge_toggle",v:b.hasClass("off")?0:1};!a.hasClass("unit");)a=a.parent();var e=a.find("h1");
e.hasClass("_busy")||(e.addClass("_busy"),$.post(AJAX_SETUP,c,function(b){e.removeClass("_busy");b.success&&(a[(c.v?"add":"remove")+"Class"]("on"),_msg("Выполнено!"))},"json"))});$("#setup_rules").length&&($(".g-save").click(function(){function b(a){c.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",top:-57,left:-6,indent:40,show:1,remove:1})}var a={op:"worker_name_save",viewer_id:RULES_VIEWER_ID,first_name:$("#first_name").val(),last_name:$("#last_name").val(),post:$("#post").val()},c=$(this);a.first_name?
a.last_name?(c.addClass("busy"),$.post(AJAX_SETUP,a,function(a){c.removeClass("busy");a.success&&_msg("Сохранено.")},"json")):(b("Не указана фамилия"),$("#last_name").focus()):(b("Не указано имя"),$("#first_name").focus())}),$("#rules_appenter")._check(function(b){$(".app-div")[(0==b?"add":"remove")+"Class"]("dn");$("#rules_info")._check(0);$("#rules_worker")._check(0);$("#rules_rules")._check(0);$("#rules_income")._check(0);$("#rules_historyshow")._check(0);$("#rules_money")._dropdown(0)}),$("#rules_money")._dropdown({spisok:[{uid:0,
title:"только свои"},{uid:1,title:"все платежи"}]}),$(".rules-save").click(function(){var b={op:"worker_rules_save",viewer_id:RULES_VIEWER_ID,rules_appenter:$("#rules_appenter").val(),rules_info:$("#rules_info").val(),rules_worker:$("#rules_worker").val(),rules_rules:$("#rules_rules").val(),rules_income:$("#rules_income").val(),rules_historyshow:$("#rules_historyshow").val(),rules_historytransfer:$("#rules_historytransfer").val(),rules_money:$("#rules_money").val()},a=$(this);a.hasClass("busy")||
(a.addClass("busy"),$.post(AJAX_SETUP,b,function(b){a.removeClass("busy");b.success&&_msg("Права сохранены.")},"json"))}),$(".dop-save").click(function(){var b={op:"worker_dop_save",viewer_id:RULES_VIEWER_ID,rules_money_procent:$("#rules_money_procent").val()},a=$(this);a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_SETUP,b,function(b){a.removeClass("busy");b.success&&_msg("Дополнительные настройки сохранены.")},"json"))}))});
