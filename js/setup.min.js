...
submit:function(){var c={op:"worker_del",viewer_id:b.attr("val")};a.process();$.post(AJAX_SETUP,c,function(b){b.success?(a.close(),_msg("Сотрудник удален."),$("#spisok").html(b.html)):a.abort()},"json")}})}).on("click","#setup-service-cartridge .add",cartridgeNew).on("click","#setup-service-cartridge .img_edit",function(){function b(){var a=_num($("#join").val()),b={op:"cartridge_edit",id:c,type_id:$("#type_id").val(),name:$("#name").val(),cost_filling:_num($("#cost_filling").val()),cost_restore:_num($("#cost_restore").val()),
cost_chip:_num($("#cost_chip").val()),join_id:a?_num($("#join_id").val()):0};b.name?a&&!b.join_id?h.err("Не выбран картридж для объединения"):(h.process(),$.post(AJAX_SETUP,b,function(a){a.success?($("#spisok").html(a.html),CARTRIDGE_SPISOK=a.cart,h.close(),_msg("Изменено!")):h.abort()},"json")):(h.err("Не указано наименование"),$("#name").focus())}for(var a=$(this),c=a.attr("val");"TR"!=a[0].tagName;)a=a.parent();var e=a.find(".name").html(),d=a.find(".type_id").val(),f=a.find(".filling").val(),
g=a.find(".restore").val(),a=a.find(".chip").val(),h=_dialog({top:40,width:400,head:"Редактирование данных картриджа",content:'<table class="setup-tab"><tr><td class="label">Вид:<td><input type="hidden" id="type_id" value="'+d+'" /><tr><td class="label"><b>Модель картриджа:</b><td><input type="text" id="name" value="'+e+'" /><tr><td class="label">Заправка:<td><input type="text" id="cost_filling" class="money" maxlength="11" value="'+f+'" /> руб.<tr><td class="label">Восстановление:<td><input type="text" id="cost_restore" class="money" maxlength="11" value="'+
g+'" /> руб.<tr><td class="label">Замена чипа:<td><input type="text" id="cost_chip" class="money" maxlength="11" value="'+a+'" /> руб.<tr><td><td><tr><td class="label">Объединить:<td><input type="hidden" id="join" /><tr class="tr-join dn"><td class="label">С картриджем:<td><input type="hidden" id="join_id" /></table>',butSubmit:"Сохранить",submit:b});$("#type_id")._select({spisok:CARTRIDGE_TYPE});$("#name").focus();$("#name,#cost_filling,#cost_restore,#cost_chip").keyEnter(b);$("#join")._check({func:function(a){$(".tr-join")[(a?
"remove":"add")+"Class"]("dn")}});e=[];for(d=0;d<CARTRIDGE_SPISOK.length;d++)f=CARTRIDGE_SPISOK[d],f.uid!=c&&e.push(f);$("#join_id")._select({width:218,write:1,title0:"Не выбрано",spisok:e})}).on("mouseleave","#setup-service-cartridge .edited",function(){$(this).css("background-color","#fff")}).on("click","#setup_invoice .add",function(){function b(){var b={op:"invoice_add",name:$("#name").val(),about:$("#about").val()};b.name?(c.process(),$.post(AJAX_SETUP,b,function(b){b.success?($(".spisok").html(b.html),
c.close(),_msg("Внесено!")):(c.abort(),a(b.text))},"json")):(a("Не указано наименование"),$("#name").focus())}function a(a){c.bottom.vkHint({msg:"<SPAN class=red>"+a+"</SPAN>",top:-47,left:100,indent:50,show:1,remove:1})}$(this);var c=_dialog({width:400,head:"Добавление нового счёта",content:'<table class="setup-tab"><tr><td class="label">Наименование:<td><input id="name" type="text" maxlength="50" /><tr><td class="label topi">Описание:<td><textarea id="about"></textarea></table>',submit:b});$("#name").focus().keyEnter(b)}).on("click",
"#setup_invoice .img_edit",function(){function b(){var b={op:"invoice_edit",id:e,name:$("#name").val(),about:$("#about").val()};b.name?(g.process(),$.post(AJAX_SETUP,b,function(b){b.success?($(".spisok").html(b.html),g.close(),_msg("Сохранено!")):(g.abort(),a(b.text))},"json")):(a("Не указано наименование"),$("#name").focus())}function a(a){g.bottom.vkHint({msg:"<SPAN class=red>"+a+"</SPAN>",top:-47,left:100,indent:50,show:1,remove:1})}for(var c=$(this);"TR"!=c[0].tagName;)c=c.parent();var e=c.attr("val"),
d=c.find(".name div").html(),f=c.find(".name pre").html();c.find(".type_id").val();var g=_dialog({width:400,head:"Редактирование данных счёта",content:'<table class="setup-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="100" value="'+d+'" /><tr><td class="label r top">Описание:<td><textarea id="about">'+f+"</textarea></table>",butSubmit:"Сохранить",submit:b});$("#name").focus().keyEnter(b)}).on("click","#setup_invoice .img_del",function(){var b=$(this),a=_dialog({top:90,
width:300,head:"Удаление счёта",content:"<center><b>Подтвердите удаление счёта.</b></center>",butSubmit:"Удалить",submit:function(){for(;"TR"!=b[0].tagName;)b=b.parent();var c={op:"setup_invoice_del",id:b.attr("val")};a.process();$.post(AJAX_SETUP,c,function(b){b.success?($(".spisok").html(b.html),a.close(),_msg("Удалено!")):a.abort()},"json")}})}).on("click","#setup_expense .add",function(){function b(){var b={op:"expense_add",name:$("#name").val(),show_worker:$("#show_worker").val()};b.name?(a.process(),
$.post(AJAX_SETUP,b,function(b){b.success?($("#spisok").html(b.html),a.close(),_msg("Внесено!"),sortable()):a.abort()},"json")):(a.bottom.vkHint({msg:"<span class=red>Не указано наименование</span>",top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}$(this);var a=_dialog({width:400,head:"Добавление новой категории расхода организации",content:'<table class="setup-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="100" /><tr><td class="label r">Список сотрудников:<td><input id="show_worker" type="hidden" /></table>',
