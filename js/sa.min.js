$(document).on("click","#sa-tovar-category .add",function(){function a(){var a={op:"tovar_category_add",name:$("#name").val()};a.name?(b.process(),$.post(AJAX_MAIN,a,function(a){a.success?($("#spisok").html(a.html),b.close(),_msg("Внесено!"),sortable()):b.abort()},"json")):(b.err("Не указано наименование"),$("#name").focus())}$(this);var b=_dialog({width:390,head:"Добавление новой категории товара",content:'<table class="sa-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" /></table>',
...
submit:a});$("#name").focus().keyEnter(a);$("#bold")._check()}).on("click",".sa-vendor .img_edit",function(){function a(){var a={op:"vendor_edit",vendor_id:b.attr("val"),name:$("#name").val(),bold:$("#bold").val()};a.name?(d.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),d.close(),_msg("Изменено!"),sortable()):d.abort()},"json")):(d.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:99,indent:50,show:1,remove:1}),$("#name").focus())}for(var b=
...
a[0].tagName;)a=a.parent();var c={op:"fault_del",id:a.attr("val")};b.process();$.post(AJAX_MAIN,c,function(a){a.success?($(".spisok").html(a.html),b.close(),_msg("Удалено!"),sortable()):b.abort()},"json")}})}).ready(function(){$(".sa-model").length&&$("#find")._search({width:300,txt:"Поиск по названию модели",enter:1,focus:1,func:function(a){var b=$(".path");b.hasClass("busy")||(a={op:"model_spisok",vendor_id:VENDOR_ID,find:escape(a)},b.addClass("busy"),$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");
a.success&&$(".spisok").html(a.html)},"json"))}})});
