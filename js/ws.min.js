...
b.name)?e():(h.process(),$.post(AJAX_WS,b,function(c){h.abort();c.success&&(VENDOR_SPISOK[a]||(VENDOR_SPISOK[a]=[]),VENDOR_SPISOK[a].push({uid:c.id,title:b.name}),VENDOR_ASS[c.id]=b.name,p._select(VENDOR_SPISOK[a])._select(c.id),r(),h.close())},"json")):e("Не указано название производителя.")}function d(){var a=p.val(),b={op:"base_model_add",device_id:q.val(),vendor_id:a,name:$("#maname").focus().val()};b.name?t(MODEL_SPISOK[a],b.name)?e():(h.process(),$.post(AJAX_WS,b,function(c){h.abort();c.success&&
