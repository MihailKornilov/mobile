...
$.fn.device=function(a){function b(){var b={op:"base_device_add",name:$("#daname").focus().val()};b.name?l(DEV_SPISOK,b.name)?e():(m.process(),$.post(AJAX_WS,b,function(c){m.abort();c.success&&(DEV_SPISOK.push({uid:c.id,title:b.name}),DEV_ASS[c.id]=name,t._select(DEV_SPISOK)._select(c.id),f(0),r.val(0)._select("remove"),a.func(q()),m.close())},"json")):e("Не указано название устройства.")}function c(){var a=t.val(),b={op:"base_vendor_add",device_id:a,name:$("#vaname").focus().val()};b.name?l(VENDOR_SPISOK[a],
b.name)?e():(m.process(),$.post(AJAX_WS,b,function(c){m.abort();c.success&&(VENDOR_SPISOK[a]||(VENDOR_SPISOK[a]=[]),VENDOR_SPISOK[a].push({uid:c.id,title:b.name}),VENDOR_ASS[c.id]=b.name,s._select(VENDOR_SPISOK[a])._select(c.id),g(),m.close())},"json")):e("Не указано название производителя.")}function d(){var a=s.val(),b={op:"base_model_add",device_id:t.val(),vendor_id:a,name:$("#maname").focus().val()};b.name?l(MODEL_SPISOK[a],b.name)?e():(m.process(),$.post(AJAX_WS,b,function(c){m.abort();c.success&&
...
b?g(0):r._select("remove");a.func(q())},funcAdd:a.vendor_funcAdd,bottom:3});s._select(a.vendor_id);a.vendor_id&&g()}function g(b){void 0!=b&&(a.model_id=b);r.val(a.model_id);r._select({width:a.width,block:1,write:1,title0:v[a.type_no],spisok:MODEL_SPISOK[s.val()],limit:50,funcAdd:a.model_funcAdd,bottom:10,func:function(){a.func(q())}});r._select(a.model_id)}function l(a,b){b=b.toLowerCase();for(var c=0;c<a.length;c++)if(a[c].title.toLowerCase()==b)return!0;return!1}function q(){return{device_id:t.val(),
vendor_id:s.val(),model_id:r.val()}}a=$.extend({width:150,func:function(){},type_no:0,device_id:0,vendor_id:0,model_id:0,device_ids:null,vendor_ids:null,model_ids:null,add:0,device_funcAdd:null,vendor_funcAdd:null,model_funcAdd:null},a);var p=$(this),h=p.attr("id"),u=["Производитель не выбран","Любой производитель"],v=["Модель не выбрана","Любая модель"],m;p.html('<input type="hidden" id="'+h+'_device" value="'+a.device_id+'"><input type="hidden" id="'+h+'_vendor" value="'+a.vendor_id+'"><input type="hidden" id="'+
...
$("#status").rightLink(0);$("#diff")._check(0);$("#zpzakaz")._radio(0);$("#dev").device({width:155,type_no:1,device_ids:G.device_ids,vendor_ids:G.vendor_ids,model_ids:G.model_ids,func:zayavSpisokLoad});$("#device_place")._select(0);$("#devstatus")._select(0);zayavSpisokLoad()}).on("click","#zayavInfo .zedit",function(){var a=_dialog({width:410,top:30,head:"Заявка №"+ZAYAV.nomer+" - Редактирование",content:'<TABLE class="zayav-info-edit"><tr><td class="label r">Клиент:\t\t<TD><INPUT type="hidden" id="client_id" value="'+
...
func:function(a){$("#place_other")[(0==a?"remove":"add")+"Class"]("dn");0==a&&$("#place_other").val("").focus()}});DEVSTATUS_SPISOK.splice(0,1);$("#dev_status")._radio({spisok:DEVSTATUS_SPISOK,light:1})}).on("click","#zayavInfo .zakaz",function(){var a=$(this),b={op:"zayav_zp_zakaz",zayav_id:ZAYAV.id,zp_id:a.parent().parent().attr("val")};$.post(AJAX_WS,b,function(b){b.success&&(a.html("Заказано!").attr("class","zakaz_ok"),_msg(b.msg))},"json")}).on("click","#zayavInfo .zakaz_ok",function(){location.href=
URL+"&p=zp&menu=3"}).on("click","#zayavInfo .zpAdd",function(){var a=_dialog({top:40,width:380,head:"Внесение новой запчасти",content:'<div class="zayav_zp_add"><CENTER>Добавление запчасти к устройству<br /><b>'+DEV_ASS[ZAYAV.device]+" "+VENDOR_ASS[ZAYAV.vendor]+" "+MODEL_ASS[ZAYAV.model]+'</b>.</CENTER><TABLE style="border-spacing:6px"><TR><TD class="label r">Наименование запчасти:<TD><input type="hidden" id="name_id"><TR><TD class="label r">Версия:<TD><input type="text" id="version" maxlength="30"><TR><TD class="label r">Цвет:<TD><input type="hidden" id="color_id"><TR><TD class="label r">Б/у:<TD><input type="hidden" id="bu"></TABLE></div>',
...
day:b,status:1,history:$("#comment").val(),from_zayav:"undefined"==typeof ZAYAV?0:ZAYAV.id,from_client:G.clientInfo?G.clientInfo.id:0};switch(d.action){case 1:d.day=$("#data").val();break;case 2:d.status=2;break;case 3:d.status=0}var e;d.action?1!=d.action&&3!=d.action||d.history?1==d.action&&d.day==b?e="Выберите новую дату.":(a.process(),$.post(AJAX_WS,d,function(b){b.success&&(a.close(),_msg("Задание отредактировано."),$("#remind_spisok").html(b.html))},"json")):e="Не указана причина.":e="Укажите новое действие.";
...
category:b.val(),about:f.val(),worker:c.val(),sum:e.val(),kassa:d.val()},l;g.about||0!=g.category?REGEXP_NUMERIC.test(g.sum)?-1==g.kassa?l="Укажите, деньги взяты из кассы или нет.":(a.process(),$.post(AJAX_WS,g,function(b){b.success&&(a.close(),_msg("Новый расход внесён."),expenseSpisok())},"json")):(l="Некорректно указана сумма.",e.focus()):(l="Выберите категорию или укажите описание.",f.focus());l&&a.bottom.vkHint({msg:'<SPAN class="red">'+l+"</SPAN>",remove:1,indent:40,show:1,top:-53,left:103,
...
$.post(AJAX_WS,a,function(a){a.success&&(_msg("Удаление произведено."),b.remove())},"json")}).on("click",".expense .img_edit",function(){var a=_dialog({top:60,width:380,head:"Редактирование расхода",load:1,butSubmit:"Сохранить",submit:function(){var l={id:b,op:"expense_edit",category:c.val(),about:g.val(),worker:d.val(),sum:f.val(),kassa:e.val()},q;l.about||0!=l.category?REGEXP_NUMERIC.test(l.sum)?-1==l.kassa?q="Укажите, деньги взяты из кассы или нет.":(a.process(),$.post(AJAX_WS,l,function(b){b.success&&
(a.close(),_msg("Расход изменён."),expenseSpisok())},"json")):(q="Некорректно указана сумма.",f.focus()):(q="Выберите категорию или укажите описание.",g.focus());q&&a.bottom.vkHint({msg:'<SPAN class="red">'+q+"</SPAN>",remove:1,indent:40,show:1,top:-53,left:103,correct:0})}}),b=$(this).attr("val"),c,d,e,f,g;$.post(AJAX_WS,{op:"expense_get",id:b},function(b){a.content.html('<TABLE id="expense-add-tab"><tr><td class="label">Категория:<TD><INPUT type="hidden" id="cat" value="'+b.category+'"><tr><td class="label">Описание:<TD><INPUT type="text" id="about" maxlength="150" value="'+
...
$("#diff")._check(zayavSpisokLoad);$("#dev").device({width:155,type_no:1,device_id:G.zayav_device,vendor_id:G.zayav_vendor,model_id:G.zayav_model,device_ids:G.device_ids,vendor_ids:G.vendor_ids,model_ids:G.model_ids,func:zayavSpisokLoad});for(n=0;n<G.place_other.length;n++){var a=G.place_other[n];DEVPLACE_SPISOK.push({uid:encodeURI(a),title:a})}DEVPLACE_SPISOK.push({uid:-1,title:"не известно",content:"<B>не известно</B>"});$("#device_place")._select({width:155,title0:"Любое местонахождение",spisok:DEVPLACE_SPISOK,
func:zayavSpisokLoad});DEVSTATUS_SPISOK.splice(0,1);DEVSTATUS_SPISOK.push({uid:-1,title:"не известно",content:"<B>не известно</B>"});$("#devstatus")._select({width:155,title0:"Любое состояние",spisok:DEVSTATUS_SPISOK,func:zayavSpisokLoad});zayavFilter()}$("#zayavAdd").length&&($("#client_id").clientSel({add:1}),$("#dev").device({width:190,add:1,device_ids:WS_DEVS,func:zayavDevSelect}),DEVPLACE_SPISOK.push({uid:0,title:'другое: <input type="text" id="place_other" class="dn" maxlength="20" />'}),$("#place")._radio({spisok:DEVPLACE_SPISOK,
...
$(".img_print").click(function(){_dialog({width:380,top:30,head:"Заявка №"+ZAYAV.nomer+" - Печать квитанции",content:'<table class="zayav-print"><tr><td class="label">Дата приёма:<td>'+PRINT.dtime+'<tr><td class="label top">Устройство:<td>'+PRINT.device+(PRINT.color?'<tr><td class="label">Цвет:<td>'+PRINT.color:"")+(PRINT.imei?'<tr><td class="label">IMEI:<td>'+PRINT.imei:"")+(PRINT.serial?'<tr><td class="label">Серийный номер:<td>'+PRINT.serial:"")+(PRINT.equip?'<tr><td class="label">Комплектация:<td>'+
