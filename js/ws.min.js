...
zayavPlace=function(a){void 0==a&&(a="");window.PLACE_OTHER||(DEVPLACE_SPISOK.push({uid:0,title:'другое: <input type="text" id="place_other" '+(a?"":'class="dn" ')+'maxlength="20" value="'+a+'" />'}),window.PLACE_OTHER=1);$("#place")._radio({spisok:DEVPLACE_SPISOK,light:1,func:function(a){$("#place_other")[(0!=a?"add":"remove")+"Class"]("dn");0==a&&$("#place_other").val("").focus()}})},zpFilter=function(){var a={find:$.trim($("#find input").val()),menu:$("#menu").val(),name:$("#zp_name").val(),device:$("#dev_device").val(),
vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),bu:$("#bu").val()},b="";a.find&&(b+=".find="+escape(a.find));0<a.menu&&(b+=".menu="+a.menu);0<a.name&&(b+=".name="+a.name);0<a.device&&(b+=".device="+a.device);0<a.vendor&&(b+=".vendor="+a.vendor);0<a.model&&(b+=".model="+a.model);0<a.bu&&(b+=".bu="+a.bu);VK.callMethod("setLocation",hashLoc+b);return a},zpSpisokLoad=function(){var a=zpFilter();a.op="zp_spisok_load";$("#mainLinks").addClass("busy");$.post(AJAX_WS,a,function(a){$("#mainLinks").removeClass("busy");
$("#zp .result").html(a.all);$("#zp .left").html(a.html)},"json")},zpImgUpdate=function(){$.post(AJAX_WS,{op:"zp_img_update",zp_id:ZP.compat_id},function(a){a.success&&$("#foto").html(a.html)},"json")},zpAvaiAdd=function(a){var b=_dialog({head:"Внесение наличия запчасти",content:'<table class="avaiAddTab"><tr><td class="left"><div class="name">'+a.name+"</div><div>"+a["for"]+'</div><div class="avai">Текущее наличие: <b>'+a.count+'</b> шт.</div><table class="inp"><tr><td class="label">Количество:<td><input type="text" id="count" maxlength="5"><tr><td class="label">Цена за ед.:<td><input type="text" id="cena" maxlength="10"><span>не обязательно</span></table><td valign="top">'+
...
version:$("#version").val(),color_id:$("#color_id").val(),bu:$("#bu").val()};0==b.name_id?a.bottom.vkHint({msg:'<SPAN class="red">Не указано наименование запчасти.</SPAN>',top:-47,left:56,show:1,remove:1,correct:0}):(a.process(),$.post(AJAX_WS,b,function(b){b.success&&(_msg("Внесение запчасти произведено."),a.close(),$("#zpSpisok").append(b.html).find("._empty").remove())},"json"))}});$("#name_id")._select({width:200,title0:"Наименование не выбрано",spisok:ZPNAME_SPISOK});$("#color_id")._select({width:130,
...
zp_id:a.attr("val"),zayav_id:ZAYAV.id};c.process();$.post(AJAX_WS,b,function(b){b.success&&(c.close(),_msg("Установка запчасти произведена."),a.after(b.zp_unit).remove(),$(".vkComment").after(b.comment).remove())},"json")}})}).on("click","#zp #bu_check",zpSpisokLoad).on("click","#zp ._next",function(){if(!$(this).hasClass("busy")){var a=$(this),b=zpFilter();b.op="zp_next";b.page=$(this).attr("val");a.addClass("busy");$.post(AJAX_WS,b,function(b){b.success?a.after(b.spisok).remove():a.removeClass("busy")},
...
func:clientSpisok}),$("#buttonCreate").vkHint({msg:'<B>Внесение нового клиента в базу.</B><br /><br />После внесения Вы попадаете на страницу с информацией о клиенте для дальнейших действий.<br /><br />Клиентов также можно добавлять при <A href="'+URL+'&p=zayav&d=add&back=client">создании новой заявки</A>.',ugol:"right",width:215,top:-38,left:-250,indent:40,delayShow:1E3,correct:0}).click(clientAdd),$("#dolg")._check(clientSpisok),$("#active")._check(clientSpisok),$("#comm")._check(clientSpisok),
$("#dolg_check").vkHint({msg:"<b>Список должников.</b><br /><br />Выводятся клиенты, у которых баланс менее 0. Также в результате отображается общая сумма долга.",ugol:"right",width:150,top:-6,left:-185,indent:20,delayShow:1E3,correct:0}));$("#clientInfo").length&&($("#dopLinks .link").click(function(){$("#dopLinks .link").removeClass("sel");$(this).addClass("sel");var a=$(this).attr("val");$(".res").css("display","zayav"==a?"block":"none");$("#zayav_filter").css("display","zayav"==a?"block":"none");
$("#zayav_spisok").css("display","zayav"==a?"block":"none");$("#money_spisok").css("display","money"==a?"block":"none");$("#remind_spisok").css("display","remind"==a?"block":"none");$("#comments").css("display","comm"==a?"block":"none");$("#histories").css("display","hist"==a?"block":"none")}),$("#status").rightLink(clientZayavSpisok),$("#diff")._check(clientZayavSpisok),$("#dev").device({width:145,type_no:1,device_ids:DEVICE_IDS,vendor_ids:VENDOR_IDS,model_ids:MODEL_IDS,func:clientZayavSpisok}));
if($("#zayav").length){window.zFind=$("#find").vkHint({msg:"Поиск производится по<br />совпадению в названии<br />модели, imei и серийном<br />номере.",ugol:"right",top:-9,left:-178,delayShow:800,correct:0})._search({width:153,focus:1,txt:"Быстрый поиск...",enter:1,func:zayavSpisokLoad});zFind.inp(G.zayav_find);$("#desc")._check(zayavSpisokLoad);$("#status").rightLink(zayavSpisokLoad);$("#diff")._check(zayavSpisokLoad);$("#dev").device({width:155,type_no:1,device_id:G.zayav_device,vendor_id:G.zayav_vendor,
model_id:G.zayav_model,device_ids:DEVICE_IDS,vendor_ids:VENDOR_IDS,model_ids:MODEL_IDS,func:zayavSpisokLoad});for(n=0;n<G.place_other.length;n++){var a=G.place_other[n];DEVPLACE_SPISOK.push({uid:encodeURI(a),title:a})}DEVPLACE_SPISOK.push({uid:-1,title:"не известно",content:"<B>не известно</B>"});$("#device_place")._select({width:155,title0:"Любое местонахождение",spisok:DEVPLACE_SPISOK,func:zayavSpisokLoad});DEVSTATUS_SPISOK.splice(0,1);DEVSTATUS_SPISOK.push({uid:-1,title:"не известно",content:"<B>не известно</B>"});
$("#devstatus")._select({width:155,title0:"Любое состояние",spisok:DEVSTATUS_SPISOK,func:zayavSpisokLoad});zayavFilter()}$("#zayavAdd").length&&($("#client_id").clientSel({add:1}),$("#dev").device({width:190,add:1,device_ids:WS_DEVS,func:zayavDevSelect}),zayavPlace(),colorSelDop(0),$(document).on("click","#fault",function(){for(var a=$(this).find("INPUT"),c=[],d=0;d<a.length;d++)if(1==a.eq(d).val()){var e=a.eq(d).attr("id").split("_")[1];c.push(FAULT_ASS[e])}$("#comm").val(c.join(", "))}),$("#comm").autosize(),
