var AJAX_WS=APP_HTML+"/ajax/ws.php?"+VALUES,AJAX_SA=APP_HTML+"/ajax/sa.php?"+VALUES,scannerWord="",scannerTime=0,scannerTimer,scannerDialog,scannerDialogShow=!1,charSpisok={48:"0",49:1,50:2,51:3,52:4,53:5,54:6,55:7,56:8,57:9,65:"A",66:"B",67:"C",68:"D",69:"E",70:"F",71:"G",72:"H",73:"I",74:"J",75:"K",76:"L",77:"M",78:"N",79:"O",80:"P",81:"Q",82:"R",83:"S",84:"T",85:"U",86:"V",87:"W",88:"X",89:"Y",90:"Z",189:"-"},modelImageGet=function(){var a={op:"model_img_get",model_id:$("#dev_model").val()},b=
$("#device_image");b.html("");0<a.model_id&&(b.addClass("busy"),$.post(AJAX_WS,a,function(a){if(a.success)b.html(a.img).find("img").on("load",function(){$(this).show().parent().removeClass("busy")})},"json"))},_valueColors=function(a,b){var c=$("#colors");if(!c.length)return!1;c.html('<input type="hidden" id="color_id" value="'+_num(a)+'" /><tt>-</tt><input type="hidden" id="color_dop" value="'+_num(b)+'" />');$("#color_id")._select({width:120,title0:"Цвет не указан",spisok:COLOR_SPISOK,func:_valueColorSet});
_valueColorSet(a)},_valueColorSet=function(a){$("#colors").find("tt")[a?"show":"hide"]();$("#color_dop")._select(a?{width:120,title0:"Цвет не указан",spisok:COLOR_SPISOK,func:function(a){$("#color_id")._select(a?COLORPRE_SPISOK:COLOR_SPISOK)._select($("#color_id").val())}}:"remove")},zayavFilter=function(){var a={op:"zayav_spisok",find:$("#find")._search("val"),sort:$("#sort").val(),desc:$("#desc").val(),status:$("#status").val(),finish:$("#day_finish").val(),diff:$("#diff").val(),zpzakaz:$("#zpzakaz").val(),
executer:$("#executer").val(),device:$("#dev_device").val(),vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),diagnost:$("#diagnost").val(),place:$("#device_place").val()},b="";"1"!=a.status&&(a.finish="0000-00-00");"1"!=a.sort&&(b+=".sort="+a.sort);"0"!=a.desc&&(b+=".desc="+a.desc);a.find?b+=".find="+escape(a.find):(0<a.status&&(b+=".status="+a.status),"0000-00-00"!=a.finish&&(b+=".finish="+a.finish),0<a.diff&&(b+=".diff=1"),0<a.zpzakaz&&(b+=".zpzakaz="+a.zpzakaz),0<a.executer&&(b+=".executer="+
a.executer),0<a.device&&(b+=".device="+a.device),0<a.vendor&&(b+=".vendor="+a.vendor),0<a.model&&(b+=".model="+a.model),0<a.diagnost&&(b+=".diagnost=1"),0!=a.place&&(b+=".place="+a.place));VK.callMethod("setLocation",hashLoc+b);_cookie(VIEWER_ID+"_zayav_find",escape(a.find));_cookie(VIEWER_ID+"_zayav_sort",a.sort);_cookie(VIEWER_ID+"_zayav_desc",a.desc);_cookie(VIEWER_ID+"_zayav_status",a.status);_cookie(VIEWER_ID+"_zayav_finish",a.finish);_cookie(VIEWER_ID+"_zayav_diff",a.diff);_cookie(VIEWER_ID+
...
"json")},zayavDevSelect=function(a){modelImageGet(a);0==a.device_id?($(".equip_spisok").html(""),$(".tr_equip").addClass("dn")):0==a.vendor_id&&0==a.model_id&&$.post(AJAX_WS,{op:"equip_check_get",device_id:a.device_id},function(a){a.spisok?($(".equip_spisok").html(a.spisok),$(".tr_equip").removeClass("dn")):($(".equip_spisok").html(""),$(".tr_equip").addClass("dn"))},"json")},zayavPlace=function(){window.PLACE_OTHER||(DEVPLACE_SPISOK.push({uid:0,title:'<div id="place-other-div">другое:<input type="text" id="place_other" class="dn" /></div>'}),
window.PLACE_OTHER=1);$("#place")._radio({spisok:DEVPLACE_SPISOK,light:1,func:function(a){$("#place_other")[(a?"add":"remove")+"Class"]("dn");a||$("#place_other").val("").focus()}})},kvitHtml=function(a){window.open(APP_HTML+"/view/kvit_html.php?"+VALUES+"&id="+a,"kvit","scrollbars=yes,resizable=yes,status=no,location=no,toolbar=no,menubar=no,width=680,height=500,left=20,top=20")},cartridgeNew=function(a,b){function c(){var c={op:"cartridge_new",type_id:$("#type_id").val(),name:$("#name").val(),cost_filling:_num($("#cost_filling").val()),
cost_restore:_num($("#cost_restore").val()),cost_chip:_num($("#cost_chip").val()),from:$("#setup-service-cartridge").length?"setup":""};c.name?(d.process(),$.post(AJAX_WS,c,function(e){e.success?("setup"==c.from?$("#spisok").html(e.spisok):(CARTRIDGE_SPISOK=e.spisok,$("#"+a)._select(e.spisok),$("#"+a)._select(e.insert_id),b(e.insert_id)),d.close(),_msg("Внесено!")):d.abort()},"json")):(d.err("Не указано наименование"),$("#name").focus())}$(this);var d=_dialog({top:20,head:"Добавление нового картриджа",
content:'<table id="cartridge-new-tab"><tr><td class="label r">Вид:<td><input type="hidden" id="type_id" value="1" /><tr><td class="label r"><b>Модель картриджа:</b><td><input type="text" id="name" /><tr><td class="label r">Заправка:<td><input type="text" id="cost_filling" class="money" maxlength="11" /> руб.<tr><td class="label r">Восстановление:<td><input type="text" id="cost_restore" class="money" maxlength="11" /> руб.<tr><td class="label r">Замена чипа:<td><input type="text" id="cost_chip" class="money" maxlength="11" /> руб.</table>',
...
$.fn.device=function(a){function b(){var b={op:"base_device_add",name:$("#daname").focus().val()};b.name?p(DEV_SPISOK,b.name)?f():(q.process(),$.post(AJAX_WS,b,function(c){q.abort();c.success&&(DEV_SPISOK.push({uid:c.id,title:b.name}),DEV_ASS[c.id]=name,t._select(DEV_SPISOK)._select(c.id),e(0),r.val(0)._select("remove"),a.func(m()),q.close())},"json")):f("Не указано название устройства.")}function c(){var a=t.val(),b={op:"base_vendor_add",device_id:a,name:$("#vaname").focus().val()};b.name?p(VENDOR_SPISOK[a],
...
(MODEL_SPISOK[a]||(MODEL_SPISOK[a]=[]),MODEL_SPISOK[a].push({uid:c.id,title:b.name}),MODEL_ASS[c.id]=b.name,r._select(MODEL_SPISOK[a])._select(c.id),q.close())},"json")):f("Не указано название модели.")}function f(a){q.bottom.vkHint({msg:'<SPAN class="red">'+(a||"Такое название уже есть в списке.")+"</SPAN>",top:-47,left:53,indent:50,show:1,remove:1})}function e(b){void 0!=b&&(a.vendor_id=b);s.val(a.vendor_id);s._select({width:a.width,block:1,title0:u[a.type_no],spisok:VENDOR_SPISOK[t.val()],func:function(b){r.val(0);
b?g(0):r._select("remove");a.func(m())},funcAdd:a.vendor_funcAdd,bottom:3});s._select(a.vendor_id);a.vendor_id&&g()}function g(b){a.no_model||(void 0!=b&&(a.model_id=b),r.val(a.model_id),r._select({width:a.width,block:1,write:1,title0:v[a.type_no],spisok:MODEL_SPISOK[s.val()],limit:50,funcAdd:a.model_funcAdd,bottom:10,func:function(){a.func(m())}}),r._select(a.model_id))}function p(a,b){if(a){b=b.toLowerCase();for(var c=0;c<a.length;c++)if(a[c].title.toLowerCase()==b)return!0}return!1}function m(){return{device_id:t.val(),
vendor_id:s.val(),model_id:r.val()}}a=$.extend({width:150,func:function(){},func_device:function(){},type_no:0,device_id:0,vendor_id:0,model_id:0,device_multiselect:0,device_ids:null,vendor_ids:null,model_ids:null,add:0,device_funcAdd:null,vendor_funcAdd:null,model_funcAdd:null,no_model:0},a);var l=$(this),h=l.attr("id"),u=["Производитель не выбран","Любой производитель"],v=["Модель не выбрана","Любая модель"],q;l.html('<input type="hidden" id="'+h+'_device" value="'+a.device_id+'"><input type="hidden" id="'+
h+'_vendor" value="'+a.vendor_id+'"><input type="hidden" id="'+h+'_model" value="'+a.model_id+'">');var t=$("#"+h+"_device"),s=$("#"+h+"_vendor"),r=$("#"+h+"_model");if(a.device_ids)for(DEV_SPISOK=[],n=0;n<a.device_ids.length;n++)l=a.device_ids[n],DEV_SPISOK.push({uid:l,title:DEV_ASS[l]});if(a.vendor_ids){h={};for(k in VENDOR_SPISOK)for(n=0;n<VENDOR_SPISOK[k].length;n++)l=VENDOR_SPISOK[k][n],0<=a.vendor_ids.indexOf(l.uid)&&(void 0==h[k]&&(h[k]=[]),h[k].push(l));VENDOR_SPISOK=h}if(a.model_ids){h={};
for(k in MODEL_SPISOK)for(n=0;n<MODEL_SPISOK[k].length;n++)l=MODEL_SPISOK[k][n],0<=a.model_ids.indexOf(l.uid)&&(h[k]||(h[k]=[]),h[k].push(l));MODEL_SPISOK=h}0<a.add&&(a.device_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвого устройства",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="daname"></table>',submit:b});$("#daname").focus().keyEnter(b)},a.vendor_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвого производителя",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="vaname"></table>',
submit:c});$("#vaname").focus().keyEnter(c)},a.model_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвой модели",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="maname"></table>',submit:d,focus:"#model_name"});$("#maname").focus().keyEnter(d)});t._select({width:a.width,block:1,multiselect:a.device_multiselect,title0:["Устройство не выбрано","Любое устройство"][a.type_no],spisok:DEV_SPISOK,func:function(b){s.val(0);r.val(0)._select("remove");
var c=t.val();"0"==c||1<c.split(",").length?s._select("remove"):e(0);a.func(m());a.func_device(b)},funcAdd:a.device_funcAdd,bottom:3});("number"==typeof a.device_id&&a.device_id||"0"!=a.device_id&&1==a.device_id.split(",").length)&&e()};
$.fn.zayavExpense=function(a){function b(a){a||(a=[0,"",""]);var c=d+f,e=c+"cat",l=c+"worker",m=c+"zp";u.append('<table id="ptab'+f+'" class="ptab" val="'+f+'"><tr><td><input type="hidden" id="'+e+'" value="'+a[0]+'" /><td class="tddop">'+(a[0]&&ZE_TXT[a[0]]?'<input type="text" class="zetxt" placeholder="описание не указано" tabindex="'+(10*f-1)+'" value="'+a[1]+'" />':"")+(a[0]&&ZE_WORKER[a[0]]?'<input type="hidden" id="'+l+'" value="'+a[1]+'" />':"")+(a[0]&&ZE_ZP[a[0]]?'<input type="hidden" id="'+
m+'" value="'+a[1]+'" />':"")+'<td class="tdsum'+(a[0]?"":" dn")+'"><input type="text" class="zesum" maxlength="6" tabindex="'+10*f+'" value="'+a[2]+'" />руб.</table>');var p=$("#ptab"+f),h=p.find(".tddop"),g=p.find(".zesum");$("#"+e)._select({width:130,disabled:0,title0:"Категория",spisok:ZE_SPISOK,func:function(a){p.find(".tdsum")[(a?"remove":"add")+"Class"]("dn");ZE_TXT[a]?(h.html('<input type="text" class="zetxt" placeholder="описание не указано" tabindex="'+(10*f-11)+'" />'),h.find(".zetxt").focus()):
(ZE_WORKER[a]?(h.html('<input type="hidden" id="'+l+'" />'),$("#"+l)._select({width:240,title0:"Сотрудник не указан",spisok:WORKER_SPISOK,func:function(a){g.focus()}})):ZE_ZP[a]?(h.html('<input type="hidden" id="'+m+'" />'),$("#"+m)._select({width:240,title0:"Запчасть не выбрана",spisok:ZAYAV.zp_avai,func:function(a){g.focus()}})):h.html(""),g.focus());g.val(1==a?ZAYAV.worker_zp:"");a&&!p.next().hasClass("ptab")&&b()}});a[0]&&ZE_WORKER[a[0]]&&$("#"+l)._select({width:240,disabled:0,title0:"Сотрудник",
spisok:WORKER_SPISOK,func:function(a){g.focus()}});a[0]&&ZE_ZP[a[0]]&&$("#"+m)._select({width:240,title0:"Запчасть не выбрана",spisok:ZAYAV.zp_avai,func:function(a){g.focus()}});f++}var c=$(this),d=c.attr("id"),f=1,e;if("string"==typeof a&&"get"==a){a=c.find(".ptab");c=[];for(e=0;e<a.length;e++){var g=a.eq(e),p=d+g.attr("val"),m=$("#"+p+"cat").val(),l=g.find(".zesum").val(),h="";if(0!=m){if(!_cena(l)&&"0"!=l)return"sum_error";ZE_TXT[m]?h=g.find(".zetxt").val():ZE_WORKER[m]?h=$("#"+p+"worker").val():
ZE_ZP[m]&&(h=$("#"+p+"zp").val());c.push(m+":"+h+":"+l)}}return c.join()}c.html('<div id="_ze-edit"></div>');var u=c.find("#_ze-edit");if("object"==typeof a)for(e=0;e<a.length;e++)b(a[e]);b();return c};
$.fn.cartridge=function(a){function b(a){d.append('<input type="hidden" class="icar" id="car'+f+'" '+(a?'value="'+a+'" ':"")+"/>");$("#car"+f)._select({width:170,bottom:4,title0:"картридж не выбран",write:1,spisok:CARTRIDGE_SPISOK,func:c,funcAdd:function(a){cartridgeNew(a,c)}})}function c(a){if(a){a=d.find(".icar");for(e=0;e<a.length;e++)if(0==a.eq(e).val())return;f++;b()}}var d=$(this);d.attr("id");var f=1,e;if("string"==typeof a&&"get"==a){a=d.find(".icar");var g=[],p;for(e=0;e<a.length;e++)p=a.eq(e).val(),
...
model_ids:Z.model_ids,device_multiselect:1,func:zayavSpisok});$("#device_place")._select(0);zayavSpisok()}).on("click","#zayav-cartridge ._next",function(){if(!$(this).hasClass("busy")){var a=$(this),b=cartridgeFilter();b.page=$(this).attr("val");a.addClass("busy");$.post(AJAX_WS,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json")}}).on("click","#zayav-cartridge .clear",function(){$("#sort")._radio(1);$("#desc")._check(0);$("#status").rightLink(0);$("#paytype")._radio(0);
$("#noschet")._check(0);cartridgeSpisok()}).on("click","#zayav-info .zedit",function(){var a=_dialog({width:420,top:30,head:"Заявка №"+ZAYAV.nomer+" - Редактирование",content:'<table class="zayav-info-edit"><tr><td class="label r">Клиент:\t\t<td><input type="hidden" id="client_id" value="'+ZAYAV.client_id+'"><tr><td class="label r top">Устройство:<td><table><td id="dev"><td id="device_image"></table><tr><td><td>'+ZAYAV.images+'<tr><td class="label r">IMEI:\t\t  <td><input type="text" id="imei" maxlength="20" value="'+
ZAYAV.imei+'"><tr><td class="label r">Серийный номер:<td><input type="text" id="serial" maxlength="30" value="'+ZAYAV.serial+'"><tr><td class="label r">Цвет:<td id="colors"><tr class="tr_equip'+(ZAYAV.equip?"":" dn")+'"><td class="label r top">Комплектация:<td class="equip_spisok">'+ZAYAV.equip+'<tr><td class="label r">Диагностика:<td><input TYPE="hidden" id="diagnost" value="'+ZAYAV.diagnost+'" /><tr><td class="label">Стоимость ремонта:<td><input type="text" class="money" id="pre_cost" maxlength="11" value="'+
(ZAYAV.pre_cost?ZAYAV.pre_cost:"")+'" /> руб.</table>',butSubmit:"Сохранить",submit:function(){var b,c={op:"zayav_edit",zayav_id:ZAYAV.id,client_id:$("#client_id").val(),device:$("#dev_device").val(),vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),imei:$.trim($("#imei").val()),serial:$.trim($("#serial").val()),color_id:$("#color_id").val(),color_dop:$("#color_dop").val(),equip:"",diagnost:$("#diagnost").val(),pre_cost:$("#pre_cost").val()};if(!$(".tr_equip").hasClass("dn")){for(var d=$(".equip_spisok input"),
f=[],e=0;e<d.length;e++){var g=d.eq(e);1==g.val()&&f.push(g.attr("id").split("_")[1])}c.equip=f.join()}0==c.deivce?b="Не выбрано устройство":0==c.client_id?b="Не выбран клиент":c.pre_cost&&!REGEXP_NUMERIC.test(c.pre_cost)?(b="Некорректно указана предварительная стоимость",$("#pre_cost").focus()):(a.process(),$.post(AJAX_WS,c,function(b){b.success?(a.close(),_msg("Данные изменены!"),document.location.reload()):a.abort()},"json"));b&&a.bottom.vkHint({msg:'<SPAN class="red">'+b+"</SPAN>",top:-47,left:107,
show:1,remove:1})}});$("#client_id").clientSel({width:267});$("#client_id_select").vkHint({msg:"Если изменяется клиент, то начисления и платежи заявки применяются на нового клиента.",width:200,top:-83,left:-2,delayShow:1500});$("#dev").device({width:190,device_id:ZAYAV.device,vendor_id:ZAYAV.vendor,model_id:ZAYAV.model,add:1,func:zayavDevSelect});modelImageGet();imageSortable();_valueColors(ZAYAV.color_id,ZAYAV.color_dop);$("#diagnost")._check()}).on("click","#zayav-info .schet-add",function(){function a(){for(var c=
"<tr><th>№<th>Наименование товара<th>Кол-во<th>Цена<th>Сумма<th>",d=0,l=0;l<f.length;l++)var h=f[l],g=h.cost*h.count,c=c+('<tr><td class="td-n">'+(l+1)+'<td class="td-name">'+h.name+'<td class="td-count">'+h.count+'<td class="td-cost">'+h.cost+'<td class="td-sum">'+g+"<td>"+(h.del?'<div val="'+l+'" class="img_del pole-del'+_tooltip("Удалить",-29)+"</div>":"")),d=d+g;e=l+1;c+='<tr><td colspan="6" class="_next" id="pole-add">Добавить позицию для счёта';$("#schet-tab").html(c);$("#itog").html("Всего наименований "+
l+", на сумму "+d+" руб.");$("#pole-add").click(b);$(".pole-del").click(function(){f.splice(_num($(this).attr("val")),1);a()})}function b(){var a=$(this),b='<tr id="tr-add"><td class="td-n">'+e+'<td class="td-name"><input type="text" id="name" /><td class="td-count"><input type="text" id="count" value="1" /><td class="td-cost"><input type="text" id="cost" /><td><div class="vkButton"><button>OK</button></div><td><div class="img_del'+_tooltip("Отменить",-32)+"</div>";a.parent().hide();$("#schet-tab").append(b);
$("#name").focus();$("#tr-add .img_del").click(function(){$("#tr-add").remove();a.parent().show()});$("#tr-add .vkButton").click(c)}function c(){var b=$.trim($("#name").val()),c=_num($("#count").val()),e=_num($("#cost").val());b?c?e?(f.push({name:b,count:c,cost:e,del:1}),a()):(d("Некорректное количество"),$("#cost").focus()):(d("Некорректная сумма"),$("#count").focus()):(d("Не указано наименование"),$("#name").focus())}function d(a){$("#name").vkHint({msg:'<span class="red">'+a+"</span>",remove:1,
indent:40,show:1,top:-58,left:404})}$(this);var f,e,g=_dialog({top:20,width:580,head:"Формирование счёта",load:1,butSubmit:"Сформировать",submit:function(){var a={op:"zayav_cartridge_schet_add",zayav_id:ZAYAV.id,spisok:f,date_create:$("#date_create").val(),dop:_num($("#dop").val()),acc:_num($("#acc").val())};a.spisok.length?a.dop?(g.process(),$.post(AJAX_WS,a,function(a){a.success?($("#cart-tab").html(a.cart),$("#schet-spisok").html(a.schet),$("#money_spisok").html(a.acc),g.close(),_msg("Новый счёт сформирован")):
g.abort()},"json")):g.err("Выберите дополнительный документ"):g.err("Не добавлено ни одной позиции")}});$.post(AJAX_WS,{op:"zayav_cartridge_schet_load",zayav_id:ZAYAV.id},function(b){b.success?(f=b.spisok,g.content.html('<div id="schet-add-tab"><table class="_spisok" id="schet-tab"></table><div id="itog"></div><table id="sa-tab"><tr><td class="label r">Дата:<td><input id="date_create" type="hidden" /><tr><td class="label r topi">Приложения:<td><input id="dop" type="hidden" /><tr><td><td><input id="acc" type="hidden" /></table></div>'),
a(),$("#date_create")._calendar({lost:1}),$("#dop")._radio({light:1,spisok:[{uid:1,title:"Накладная"},{uid:2,title:"Акт выполненных работ"}]}),$("#acc")._check({name:"Произвести начисление по счёту"})):g.loadError()},"json")}).on("click","#zayav-info .acc_add",function(){function a(){var a,d={op:"zayav_accrual_add",zayav_id:ZAYAV.id,sum:$("#sum").val(),prim:$("#prim").val(),status:$("#acc_status").val(),remind:$("#acc_remind").val(),remind_txt:$("#reminder_txt").val(),remind_day:$("#reminder_day").val()};
...
ZAYAV.pay_type+'" /></table>',butSubmit:"Сохранить",submit:function(){var b={op:"zayav_cartridge_edit",zayav_id:ZAYAV.id,client_id:_num($("#client_id").val()),count:_num($("#count").val()),pay_type:_num($("#pay_type").val())};b.client_id?b.count?b.pay_type?(a.process(),$.post(AJAX_WS,b,function(b){b.success?(a.close(),_msg("Заявка изменена"),document.location.reload()):a.abort()},"json")):a.err("Укажите вид расчёта"):a.err("Не указано количество картриджей"):a.err("Не указан клиент")}});$("#client_id").clientSel({add:1});
...
g=b.find(".chip").val(),p=b.find(".cost").html(),b=b.find("u").html(),m=_dialog({width:470,top:30,head:"Действия по картриджу",content:'<table id="cart-edit-tab"><tr><td class="label">Картридж:<td><input type="hidden" id="cart_id" value="'+d+'" /><tr><td class="label topi">Действие:<td><input type="hidden" id="filling" value="'+f+'" /><input type="hidden" id="restore" value="'+e+'" /><input type="hidden" id="chip" value="'+g+'" /><tr><td class="label">Стоимость работ:<td><input type="text" class="money" id="cost" value="'+
...
id:b};c.process();$.post(AJAX_WS,a,function(a){a.success?(c.close(),$("#cart-tab").html(a.html),_msg("Удалено.")):c.abort()},"json")}})}).ready(function(){$("#zayav").length&&($("#find").vkHint({msg:"Поиск производится по<br />совпадению в названии<br />модели, imei и серийном<br />номере.",ugol:"right",top:-9,left:-178,delayShow:800})._search({width:153,focus:1,txt:"Быстрый поиск...",enter:1,func:zayavSpisok}).inp(Z.find),$("#desc")._check(zayavSpisok),$("#status").rightLink(zayavSpisok),$("#diagnost")._check(zayavSpisok),
$("#diff")._check(zayavSpisok),WORKER_SPISOK.push({uid:-1,title:"Не назначен",content:"<b>Не назначен</b>"}),$("#executer")._select({width:155,title0:"не указан",spisok:WORKER_SPISOK,func:zayavSpisok}),$("#dev").device({width:155,type_no:1,device_id:Z.device_id,vendor_id:Z.vendor_id,model_id:Z.model_id,device_multiselect:1,device_ids:Z.device_ids,vendor_ids:Z.vendor_ids,model_ids:Z.model_ids,func:zayavSpisok}),$("#device_place")._select({width:155,title0:"Любое местонахождение",spisok:DEVPLACE_SPISOK,
func:zayavSpisok}),Z.cookie_id&&(VK.callMethod("scrollWindow",_cookie("zback_scroll")),$("#u"+Z.cookie_id).css("opacity",0.1).delay(400).animate({opacity:1},700)),zayavFilter());$("#zayavAdd").length&&($("#client_id").clientSel({add:1}),$("#dev").device({width:190,add:1,device_ids:WS_DEVS,func:zayavDevSelect}),zayavPlace(),_valueColors(0),$("#comm").autosize(),$(".vkCancel").click(function(){location.href=URL+"&p="+$(this).attr("val")}),$(".vkButton").click(function(){var a=$(this),b={op:"zayav_add",
client_id:$("#client_id").val(),device:$("#dev_device").val(),vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),equip:"",place:$("#place").val(),place_other:$("#place_other").val(),imei:$("#imei").val(),serial:$("#serial").val(),color:$("#color_id").val(),color_dop:$("#color_dop").val(),diagnost:$("#diagnost").val(),comm:$("#comm").val(),pre_cost:$("#pre_cost").val(),day_finish:$("#day_finish").val()};if(!a.hasClass("busy")){if(!$(".tr_equip").hasClass("dn")){for(var c=$(".equip_spisok input"),
d=[],f=0;f<c.length;f++){var e=c.eq(f);1==e.val()&&d.push(e.attr("id").split("_")[1])}b.equip=d.join()}c="";0==b.client_id?c="Не выбран клиент":0==b.device?c="Не выбрано устройство":"-1"==b.place||0==b.place&&!b.place_other?c="Не указано местонахождение устройства":!b.pre_cost||REGEXP_NUMERIC.test(b.pre_cost)&&0!=b.pre_cost?"0000-00-00"==b.day_finish?c="Не указан срок выполнения ремонта":(0<b.place&&(b.place_other=""),a.addClass("busy"),$.post(AJAX_WS,b,function(b){b.success?location.href=URL+"&p=zayav&d=info&id="+
