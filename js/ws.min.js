...
vendor_id:s.val(),model_id:r.val()}}a=$.extend({width:150,func:function(){},type_no:0,device_id:0,vendor_id:0,model_id:0,device_multiselect:0,device_ids:null,vendor_ids:null,model_ids:null,add:0,device_funcAdd:null,vendor_funcAdd:null,model_funcAdd:null},a);var l=$(this),h=l.attr("id"),u=["Производитель не выбран","Любой производитель"],v=["Модель не выбрана","Любая модель"],q;l.html('<input type="hidden" id="'+h+'_device" value="'+a.device_id+'"><input type="hidden" id="'+h+'_vendor" value="'+a.vendor_id+
'"><input type="hidden" id="'+h+'_model" value="'+a.model_id+'">');var t=$("#"+h+"_device"),s=$("#"+h+"_vendor"),r=$("#"+h+"_model");if(a.device_ids)for(DEV_SPISOK=[],n=0;n<a.device_ids.length;n++)l=a.device_ids[n],DEV_SPISOK.push({uid:l,title:DEV_ASS[l]});if(a.vendor_ids){h={};for(k in VENDOR_SPISOK)for(n=0;n<VENDOR_SPISOK[k].length;n++)l=VENDOR_SPISOK[k][n],0<=a.vendor_ids.indexOf(l.uid)&&(void 0==h[k]&&(h[k]=[]),h[k].push(l));VENDOR_SPISOK=h}if(a.model_ids){h={};for(k in MODEL_SPISOK)for(n=0;n<
MODEL_SPISOK[k].length;n++)l=MODEL_SPISOK[k][n],0<=a.model_ids.indexOf(l.uid)&&(h[k]||(h[k]=[]),h[k].push(l));MODEL_SPISOK=h}0<a.add&&(a.device_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвого устройства",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="daname"></table>',submit:b});$("#daname").focus().keyEnter(b)},a.vendor_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвого производителя",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="vaname"></table>',
submit:c});$("#vaname").focus().keyEnter(c)},a.model_funcAdd=function(){q=_dialog({width:300,head:"Добавление нoвой модели",content:'<table class="device-add-tab"><tr><td class="label">Название:<td><input type="text" id="maname"></table>',submit:e,focus:"#model_name"});$("#maname").focus().keyEnter(e)});t._select({width:a.width,block:1,multiselect:a.device_multiselect,title0:["Устройство не выбрано","Любое устройство"][a.type_no],spisok:DEV_SPISOK,func:function(){s.val(0);r.val(0)._select("remove");
var b=t.val();"0"==b||1<b.split(",").length?s._select("remove"):d(0);a.func(m())},funcAdd:a.device_funcAdd,bottom:3});"0"!=a.device_id&&1==a.device_id.split(",").length&&d()};
