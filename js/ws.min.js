...
ZAYAV.serial+'"><tr><td class="label r">Цвет:<td><input type="hidden" id="color_id" value="'+ZAYAV.color_id+'" /><span class="color_dop dn"><tt>-</tt><input TYPE="hidden" id="color_dop" value="'+ZAYAV.color_dop+'" /></span><tr class="tr_equip'+(ZAYAV.equip?"":" dn")+'"><td class="label r top">Комплектация:<td class="equip_spisok">'+ZAYAV.equip+'<tr><td class="label r">Диагностика:<td><input TYPE="hidden" id="diagnost" value="'+ZAYAV.diagnost+'" /><tr><td class="label">Стоимость ремонта:<td><input type="text" class="money" id="pre_cost" maxlength="11" value="'+
(ZAYAV.pre_cost?ZAYAV.pre_cost:"")+'" /> руб.</table>',butSubmit:"Сохранить",submit:function(){var b,c={op:"zayav_edit",zayav_id:ZAYAV.id,client_id:$("#client_id").val(),device:$("#dev_device").val(),vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),imei:$.trim($("#imei").val()),serial:$.trim($("#serial").val()),color_id:$("#color_id").val(),color_dop:$("#color_dop").val(),equip:"",diagnost:$("#diagnost").val(),pre_cost:$("#pre_cost").val()};if(!$(".tr_equip").hasClass("dn")){for(var e=$(".equip_spisok input"),
f=[],d=0;d<e.length;d++){var g=e.eq(d);1==g.val()&&f.push(g.attr("id").split("_")[1])}c.equip=f.join()}0==c.deivce?b="Не выбрано устройство":0==c.client_id?b="Не выбран клиент":c.pre_cost&&!REGEXP_NUMERIC.test(c.pre_cost)?(b="Некорректно указана предварительная стоимость",$("#pre_cost").focus()):(a.process(),$.post(AJAX_WS,c,function(b){b.success?(a.close(),_msg("Данные изменены!"),document.location.reload()):a.abort()},"json"));b&&a.bottom.vkHint({msg:'<SPAN class="red">'+b+"</SPAN>",top:-47,left:107,
show:1,remove:1})}});$("#client_id").clientSel({width:267});$("#client_id_select").vkHint({msg:"Если изменяется клиент, то начисления и платежи заявки применяются на нового клиента.",width:200,top:-83,left:-2,delayShow:1500});$("#dev").device({width:190,device_id:ZAYAV.device,vendor_id:ZAYAV.vendor,model_id:ZAYAV.model,add:1,func:zayavDevSelect});modelImageGet();imageSortable();colorSelDop(ZAYAV.color_id);$("#diagnost")._check()}).on("click","#zayav-info .schet-add",function(){function a(){for(var c=
"<tr><th>№<th>Наименование товара<th>Кол-во<th>Цена<th>Сумма<th>",e=0,l=0;l<f.length;l++)var h=f[l],g=h.cost*h.count,c=c+('<tr><td class="td-n">'+(l+1)+'<td class="td-name">'+h.name+'<td class="td-count">'+h.count+'<td class="td-cost">'+h.cost+'<td class="td-sum">'+g+"<td>"+(h.del?'<div val="'+l+'" class="img_del pole-del'+_tooltip("Удалить",-29)+"</div>":"")),e=e+g;d=l+1;c+='<tr><td colspan="6" class="_next" id="pole-add">Добавить позицию для счёта';$("#schet-tab").html(c);$("#itog").html("Всего наименований "+
l+", на сумму "+e+" руб.");$("#pole-add").click(b);$(".pole-del").click(function(){f.splice(_num($(this).attr("val")),1);a()})}function b(){var a=$(this),b='<tr id="tr-add"><td class="td-n">'+d+'<td class="td-name"><input type="text" id="name" /><td class="td-count"><input type="text" id="count" value="1" /><td class="td-cost"><input type="text" id="cost" /><td><div class="vkButton"><button>OK</button></div><td><div class="img_del'+_tooltip("Отменить",-32)+"</div>";a.parent().hide();$("#schet-tab").append(b);
$("#name").focus();$("#tr-add .img_del").click(function(){$("#tr-add").remove();a.parent().show()});$("#tr-add .vkButton").click(c)}function c(){var b=$.trim($("#name").val()),c=_num($("#count").val()),d=_num($("#cost").val());b?c?d?(f.push({name:b,count:c,cost:d,del:1}),a()):(e("Некорректное количество"),$("#cost").focus()):(e("Некорректная сумма"),$("#count").focus()):(e("Не указано наименование"),$("#name").focus())}function e(a){$("#name").vkHint({msg:'<span class="red">'+a+"</span>",remove:1,
indent:40,show:1,top:-58,left:404})}$(this);var f,d,g=_dialog({top:20,width:580,head:"Формирование счёта",load:1,butSubmit:"Сформировать",submit:function(){var a={op:"zayav_cartridge_schet_add",zayav_id:ZAYAV.id,spisok:f,date_create:$("#date_create").val(),dop:_num($("#dop").val()),acc:_num($("#acc").val())};a.spisok.length?a.dop?(g.process(),$.post(AJAX_WS,a,function(a){a.success?($("#cart-tab").html(a.cart),$("#schet-spisok").html(a.schet),$("#money_spisok").html(a.acc),g.close(),_msg("Новый счёт сформирован")):
