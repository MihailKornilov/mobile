...
zayavPlace();$(".st").click(function(){var a=$(this),c=a.attr("val");a.parent().find(".st").hide();a.show();$("#zs-status").val(c);$("#zs-tab").show();1==c&&$("#zs-srok").removeClass("dn")})}).on("click","#zayav-info .cartridge_status",function(){function a(){var a={op:"zayav_cartridge_status",zayav_id:ZAYAV.id,status:_num($("#zs-status").val())};b.process();$.post(AJAX_MAIN,a,function(a){a.success?(b.close(),_msg("Изменения сохранены."),document.location.reload()):b.abort()},"json")}var b=_dialog({top:30,
width:420,head:"Изменение статуса заявки",content:'<div id="zayav-status">'+(1!=ZAYAV.status?'<div class="st c1" val="1">Ожидает выполнения<div class="about">Возобновление работы по заявке.</div></div>':"")+(2!=ZAYAV.status?'<div class="st c2" val="2">Выполнено<div class="about">Заявка выполнена успешно.<br />Не забудьте расписать расходы по заявке, проверьте начисления.<br />Добавьте напоминание, если необходимо.</div></div>':"")+(3!=ZAYAV.status?'<div class="st c3" val="3">Заявка отменена<div class="about">Отмена заявки по какой-либо причине.</div></div>':
...
"#zayav-info #cart-add",zayavInfoCartridgeAdd).on("click","#zayav-info .cart-edit",function(){function a(){var a=0,b=_num($("#cart_id").val());1==$("#filling").val()&&(a=CARTRIDGE_FILLING[b]);1==$("#restore").val()&&(a+=CARTRIDGE_RESTORE[b]);1==$("#chip").val()&&(a+=CARTRIDGE_CHIP[b]);$("#cost").val(a)}for(var b=$(this);"TR"!=b[0].tagName;)b=b.parent();var c=b.attr("val"),d=b.find(".cart_id").val(),f=b.find(".filling").val(),e=b.find(".restore").val(),h=b.find(".chip").val(),p=b.find(".cost").html(),
b=b.find("u").html(),r=_dialog({width:470,top:30,head:"Действия по картриджу",content:'<table id="cart-edit-tab"><tr><td class="label">Картридж:<td><input type="hidden" id="cart_id" value="'+d+'" /><tr><td class="label topi">Действие:<td><input type="hidden" id="filling" value="'+f+'" /><input type="hidden" id="restore" value="'+e+'" /><input type="hidden" id="chip" value="'+h+'" /><tr><td class="label">Стоимость работ:<td><input type="text" class="money" id="cost" value="'+p+'" /> руб.<tr><td class="label">Примечание:<td><input type="text" id="prim" value="'+
b+'" /></table>',butSubmit:"Сохранить",submit:function(){var a={op:"zayav_info_cartridge_edit",id:c,cart_id:_num($("#cart_id").val()),filling:$("#filling").val(),restore:$("#restore").val(),chip:$("#chip").val(),cost:$("#cost").val(),prim:$("#prim").val()};a.cart_id?(r.process(),$.post(AJAX_MAIN,a,function(a){a.success?(r.close(),$("#cart-tab").html(a.html),_msg("Изменено.")):r.abort()},"json")):r.err("Не выбран картридж")}});$("#cart_id")._select({write:1,spisok:CARTRIDGE_SPISOK,func:a});$("#filling")._check({name:"Заправка",
func:a});$("#restore")._check({name:"Восстановление",func:a});$("#chip")._check({name:"Замена чипа",func:a})}).on("click","#zayav-info .cart-del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b=a.attr("val"),c=_dialog({head:"Удаление картриджа",content:"<center>Подтвердите удаление картриджа.</center>",butSubmit:"Удалить",submit:function(){var a={op:"zayav_info_cartridge_del",id:b};c.process();$.post(AJAX_MAIN,a,function(a){a.success?(c.close(),$("#cart-tab").html(a.html),_msg("Удалено.")):
c.abort()},"json")}})}).ready(function(){$("#zayav").length&&($("#find").vkHint({msg:"Поиск производится по<br />совпадению в названии<br />модели, imei и серийном<br />номере.",ugol:"right",top:-9,left:-178,delayShow:800})._search({width:153,focus:1,txt:"Быстрый поиск...",enter:1,func:zayavSpisok}).inp(ZAYAV.find),$("#sort")._radio(zayavSpisok),$("#desc")._check(zayavSpisok),$("#status").rightLink(zayavSpisok),$("#diagnost")._check(zayavSpisok),$("#diff")._check(zayavSpisok),$("#zpzakaz")._radio(zayavSpisok),
WORKER_SPISOK.push({uid:-1,title:"Не назначен",content:"<b>Не назначен</b>"}),$("#executer")._select({width:155,title0:"не указан",spisok:WORKER_SPISOK,func:zayavSpisok}),$("#dev").device({width:155,type_no:1,device_id:ZAYAV.device,vendor_id:ZAYAV.vendor,model_id:ZAYAV.model,device_multiselect:1,device_ids:Z.device_ids,vendor_ids:Z.vendor_ids,model_ids:Z.model_ids,func:function(a){ZAYAV.device=a.device_id;ZAYAV.vendor=a.vendor_id;ZAYAV.model=a.model_id;zayavSpisok()}}),$("#place")._select({width:155,
title0:"Любое местонахождение",spisok:DEVPLACE_SPISOK,func:zayavSpisok}),Z.cookie_id&&(VK.callMethod("scrollWindow",_cookie("zback_scroll")),$("#u"+Z.cookie_id).css("opacity",0.1).delay(400).animate({opacity:1},700)));$("#zayavAdd").length&&($("#client_id").clientSel({add:1}),$("#dev").device({width:190,add:1,device_ids:WS_DEVS,func:zayavDevSelect}),zayavPlace(),_valueColors(0),$("#comm").autosize(),$(".vkCancel").click(function(){location.href=URL+"&p="+$(this).attr("val")}),$(".vkButton").click(function(){var a=
$(this),b={op:"zayav_add",client_id:$("#client_id").val(),device:$("#dev_device").val(),vendor:$("#dev_vendor").val(),model:$("#dev_model").val(),equip:"",place:$("#place").val(),place_other:$("#place_other").val(),imei:$("#imei").val(),serial:$("#serial").val(),color:$("#color_id").val(),color_dop:$("#color_dop").val(),diagnost:$("#diagnost").val(),comm:$("#comm").val(),pre_cost:$("#pre_cost").val(),day_finish:$("#day_finish").val()};if(!a.hasClass("busy")){if(!$(".tr_equip").hasClass("dn")){for(var c=
$(".equip_spisok input"),d=[],f=0;f<c.length;f++){var e=c.eq(f);1==e.val()&&d.push(e.attr("id").split("_")[1])}b.equip=d.join()}c="";0==b.client_id?c="Не выбран клиент":0==b.device?c="Не выбрано устройство":"-1"==b.place||0==b.place&&!b.place_other?c="Не указано местонахождение устройства":!b.pre_cost||REGEXP_NUMERIC.test(b.pre_cost)&&0!=b.pre_cost?"0000-00-00"==b.day_finish?c="Не указан срок выполнения ремонта":(0<b.place&&(b.place_other=""),a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?
